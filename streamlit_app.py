"""ResumeMapper AI - self-contained, evidence-grounded resume matcher.
...
st.set_page_config(page_title="ResumeMapper — Evidence-first career intelligence", page_icon="◎", layout="wide")
...
def report_pdf(candidate: CandidateProfile, job_title: str, company: str, report: MatchReport) -> bytes:
    ...
    document = SimpleDocTemplate(
        ...
        title="ResumeMapper Explainable Match Report",
    )
    ...
    story: list[Any] = [
        ...
        Paragraph("ResumeMapper Explainable Match Report", styles["Title"]),
...
def improved_resume_docx(candidate: CandidateProfile, optimized: OptimizedResume) -> bytes:
    ...
    document.core_properties.subject = "Evidence-grounded tailored resume generated by ResumeMapper"
...
def improved_resume_pdf(candidate: CandidateProfile, optimized: OptimizedResume) -> bytes:
    ...
    regular_font = "ResumeMapperSans"
    bold_font = "ResumeMapperSans-Bold"
    ...
    document = SimpleDocTemplate(
        ...
        subject="Evidence-grounded tailored resume generated by ResumeMapper",
        allowSplitting=1,
    )
    styles = getSampleStyleSheet()
    styles.add(
...
def resumemapper_logo_svg(css_class: str = "hs-logo") -> str:
    return (
        f'<svg class="{css_class}" viewBox="0 0 64 64" role="img" aria-label="ResumeMapper logo">'
        '<defs><linearGradient id="resumeMapperGradient" x1="8" y1="7" x2="57" y2="58" gradientUnits="userSpaceOnUse">'
        '<stop stop-color="#2563EB"/><stop offset="1" stop-color="#0891B2"/></linearGradient></defs>'
        '<rect x="4" y="4" width="56" height="56" rx="17" fill="url(#resumeMapperGradient)"/>'
        '<path d="M32 13C23.7 13 17 19.5 17 27.7C17 38.1 32 51 32 51S47 38.1 47 27.7C47 19.5 40.3 13 32 13Z" fill="none" stroke="white" stroke-width="4.5" stroke-linejoin="round"/>'
...
def inject_styles() -> None:
    st.markdown(
        ...
        .hs-journey-title { text-align:center; color:var(--navy); font-size:1.35rem; font-weight:850; margin:1.2rem 0 .15rem; }
        .hs-journey-subtitle { text-align:center; color:var(--muted); font-size:.88rem; margin-bottom:.9rem; }
        .hs-step-shell { min-height:138px; border:1px solid var(--line); border-radius:20px; padding:1.1rem; background:linear-gradient(145deg,#fff,#f3f8ff); box-shadow:0 12px 30px rgba(15,35,68,.07); animation:hs-enter .55s ease-out both; }
        .hs-step-shell.two { animation-delay:.1s; }
        .hs-step-shell.three { animation-delay:.2s; }
        .hs-step-shell .number { display:inline-grid; place-items:center; width:2rem; height:2rem; border-radius:10px; color:#fff; font-weight:900; background:linear-gradient(135deg,#2563eb,#0891b2); }
        .hs-step-shell strong { display:block; color:var(--navy); font-size:1.02rem; margin:.65rem 0 .2rem; }
        .hs-step-shell span { color:var(--muted); font-size:.79rem; line-height:1.45; }
        .hs-nav-label { color:#64748b; font-size:.7rem; font-weight:850; letter-spacing:.12em; text-transform:uppercase; margin:.15rem 0 .35rem; }
        .hs-next-card { border:1px solid #bfdbfe; border-radius:16px; padding:.9rem 1rem; background:linear-gradient(120deg,#eff6ff,#ecfdf5); margin:1rem 0 .65rem; }
        .hs-next-card strong { color:var(--navy); }
        .hs-next-card span { color:var(--muted); font-size:.82rem; }
...
def initialize_state() -> None:
    defaults = {
        ...
        "guide_messages": [
            {
                "role": "assistant",
                "content": "Hi! I’m the ResumeMapper Guide. Ask me about analyzing, improving, or downloading your résumé.",
            }
        ],
    }
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="hs-side-brand">
              {resumemapper_logo_svg()}
              <div><div class="hs-side-name">ResumeMapper</div><div class="hs-side-caption">Smarter, evidence-first applications</div></div>
            </div>
            <div class="hs-side-guide">
              <strong>Your application journey</strong>
              <div class="hs-side-step"><span>1</span><div>Analyze</div></div>
              <div class="hs-side-step"><span>2</span><div>Fix priority gaps</div></div>
              <div class="hs-side-step"><span>3</span><div>Tailor &amp; download</div></div>
            </div>
            <div class="hs-side-trust">Private by design · Evidence before claims</div>
            """,
            unsafe_allow_html=True,
        )
...
def render_analyze_input(settings: Settings, use_ai: bool) -> None:
    ...
    try:
        ...
        st.session_state.report_focus = "overview"
        st.rerun()
...
def render_tracker() -> None:
    ...
    try:
        ...
        action_cols[1].download_button(
            ...
            file_name="resumemapper_application_tracker.csv",
            mime="text/csv",
            width="stretch",
        )
...
def _render_tailor_and_exports(
    ...
    if optimized:
        ...
        st.markdown('<div class="hs-next-card"><strong>Would you like to add this application to your tracker?</strong><br><span>Save the role, company, match score, and tailored résumé status.</span></div>', unsafe_allow_html=True)
        tracker_choice = st.columns(2)
        if tracker_choice[0].button("Yes, add to tracker", key="save_tailored_to_tracker", type="primary", width="stretch"):
            created, message = save_current_job_to_tracker(report, "Tailored")
            (st.success if created else st.info)(message)
        if tracker_choice[1].button("Not now", key="skip_tailored_tracker", width="stretch"):
            st.info("No problem—your files are ready to download above.")
        st.warning("Review the preview carefully. ResumeMapper improves wording but cannot verify facts not stated in your résumé.")
    ...
    export_cols[0].download_button(
        ...
        report_csv(report),
        file_name="resumemapper_requirement_evidence.csv",
        mime="text/csv",
        width="stretch",
    )
    try:
        ...
        export_cols[1].download_button(
            ...
            pdf_bytes,
            file_name="resumemapper_match_report.pdf",
            mime="application/pdf",
            width="stretch",
        )
...
def render_report(settings: Settings, use_ai: bool) -> None:
    ...
    st.header("ResumeMapper Match Report")
    job_line = " · ".join(
        value
        for value in (
            st.session_state.job_title,
    ...
    focus = st.session_state.get("report_focus", "overview")
    section_cols = st.columns(3)
    if section_cols[0].button("Overview", type="primary" if focus == "overview" else "secondary", width="stretch"):
        st.session_state.report_focus = "overview"
        st.rerun()
    if section_cols[1].button("Fix priority gaps", type="primary" if focus == "gaps" else "secondary", width="stretch"):
        st.session_state.report_focus = "gaps"
        st.rerun()
    if section_cols[2].button("Explore evidence", type="primary" if focus == "evidence" else "secondary", width="stretch"):
        st.session_state.report_focus = "evidence"
        st.rerun()
    ...
    if focus == "overview":
        ...
        st.markdown('<div class="hs-next-card"><strong>Next: improve your strongest opportunities</strong><br><span>Open the prioritized action list before tailoring your résumé.</span></div>', unsafe_allow_html=True)
        if st.button("Review priority gaps →", type="primary", width="stretch"):
            st.session_state.report_focus = "gaps"
            st.rerun()
    elif focus == "gaps":
        _render_priority_gaps(candidate, report)
        st.markdown('<div class="hs-next-card"><strong>Ready to create your tailored résumé?</strong><br><span>Only verified experience will be used.</span></div>', unsafe_allow_html=True)
        if st.button("Continue to Tailor & Download →", type="primary", width="stretch"):
            st.session_state.current_view = "tailor"
            st.rerun()
    else:
...
def render_method() -> None:
    st.markdown('<div class="hs-eyebrow">How ResumeMapper works</div>', unsafe_allow_html=True)
    st.header("Transparent matching—not a mystery score")
    st.write(
        "ResumeMapper breaks the job description into individual requirements, finds the strongest résumé evidence for "
    ...
    st.warning("ResumeMapper is decision support—not an employer ATS score, hiring decision, or legal/immigration opinion.")


def resumemapper_guide_response(prompt: str) -> str:
    ...
    if any(term in question for term in ("score", "percentage", "match", "direct", "related", "uncertain", "missing")):
        return "ResumeMapper checks every job requirement separately. **Direct** means clearly proven, **Related** means transferable evidence, **Uncertain** needs confirmation, and **Missing** means no specific résumé evidence was found. Required qualifications count more than preferred ones."
    if any(term in question for term in ("tailor", "improve", "rewrite", "resume", "résumé")):
        return "Create a match report first, then open **3 · Tailor & Improve**. ResumeMapper improves clarity and supported keyword placement without adding experience, tools, or credentials that are not in your résumé."
    ...
    if any(term in question for term in ("api", "key", "free", "cost", "charge")):
        return "You do **not** need an API key to use ResumeMapper. The public app includes deterministic parsing, evidence matching, recommendations, tracking, and document exports."
    if any(term in question for term in ("discover", "linkedin", "indeed", "google", "job search")):
        return "Open **2 · Discover**, enter target titles and locations, and ResumeMapper will build direct searches for LinkedIn, Indeed, and Google Jobs."
...
def _append_guide_exchange(prompt: str) -> None:
    ...
    st.session_state.guide_messages.append({"role": "assistant", "content": resumemapper_guide_response(cleaned)})
...
def render_guide() -> None:
    ...
    st.header("Ask the ResumeMapper Guide")
    st.markdown(
        '<div class="hs-guide-hero"><strong>Instant help—no API key required</strong><span>This guided assistant answers common product and workflow questions using built-in ResumeMapper knowledge.</span></div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("guide_question_form", clear_on_submit=True, border=False):
        ...
        submitted = st.form_submit_button("Ask ResumeMapper", type="primary", width="stretch")
    if submitted and clean_line(question):
        _append_guide_exchange(question)
        st.rerun()


def _go_to(view: str, report_focus: str | None = None) -> None:
    st.session_state.current_view = view
    if report_focus:
        st.session_state.report_focus = report_focus


def render_navigation() -> None:
    st.markdown('<div class="hs-nav-label">Explore ResumeMapper</div>', unsafe_allow_html=True)
    labels = [("Home", "home"), ("Analyze", "analyze"), ("Discover", "discover"), ("Tracker", "tracker"), ("How it works", "method"), ("Ask ResumeMapper", "guide")]
    columns = st.columns(len(labels))
    current = st.session_state.current_view
    for column, (label, view) in zip(columns, labels):
        if column.button(label, type="primary" if current == view else "secondary", key=f"nav_{view}", width="stretch"):
            _go_to(view)
            st.rerun()
    ...
    st.markdown(
        ...
        <section class="hs-hero">
          ...
          <div class="hs-hero-top">
            <div class="hs-hero-brand">{resumemapper_logo_svg("hs-logo hs-logo-hero")}<div><strong>ResumeMapper</strong><span>Career Intelligence</span></div></div>
            <div class="hs-privacy-chip">◇ Evidence before claims</div>
          </div>
          <h1>Turn your experience into a <span>stronger application.</span></h1>
          <p class="hs-hero-copy">See your true fit. Fix the right gaps. Download a tailored résumé.</p>
          <div class="hs-hero-pills"><span>Free to use</span><span>No sign-up</span><span>No invented experience</span></div>
        </section>
        <div class="hs-journey-title">Three steps. One confident application.</div>
        <div class="hs-journey-subtitle">Choose a step to begin or continue.</div>
        """,
        unsafe_allow_html=True,
    )
    columns = st.columns(3, gap="large")
    cards = [
        ("one", "1", "Analyze", "Upload your résumé and compare it with a job."),
        ("two", "2", "Understand & fix", "Open your explainable report and priority gaps."),
        ("three", "3", "Tailor & download", "Create polished Word and PDF versions."),
    ]
    for column, (tone, number, title, copy) in zip(columns, cards):
        with column:
            st.markdown(f'<div class="hs-step-shell {tone}"><span class="number">{number}</span><strong>{title}</strong><span>{copy}</span></div>', unsafe_allow_html=True)
    actions = st.columns(3, gap="large")
    if actions[0].button("Start analysis →", type="primary", key="journey_analyze", width="stretch"):
        _go_to("analyze")
        st.rerun()
    if actions[1].button("Open match report →", key="journey_report", width="stretch"):
        if st.session_state.report:
            _go_to("report", "overview")
        else:
            _go_to("analyze")
            st.toast("Create an analysis first, then your report will open automatically.")
        st.rerun()
    if actions[2].button("Tailor résumé →", key="journey_tailor", width="stretch"):
        if st.session_state.report:
            _go_to("tailor")
        else:
            _go_to("analyze")
            st.toast("Create an analysis first to unlock tailoring.")
        st.rerun()
...
def main() -> None:
    ...
    if view == "home":
        ...
        render_guide()
    st.markdown('<div class="hs-footer"><strong>ResumeMapper</strong> · Prove your experience. Find your fit.</div>', unsafe_allow_html=True)
