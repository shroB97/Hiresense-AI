"""MeritLoom AI - self-contained, evidence-grounded resume matcher.
...
st.set_page_config(page_title="MeritLoom — Evidence-first career intelligence", page_icon="◎", layout="wide")
...
def report_pdf(candidate: CandidateProfile, job_title: str, company: str, report: MatchReport) -> bytes:
    ...
    document = SimpleDocTemplate(
        ...
        title="MeritLoom Explainable Match Report",
    )
    ...
    story: list[Any] = [
        ...
        Paragraph("MeritLoom Explainable Match Report", styles["Title"]),
...
def improved_resume_docx(candidate: CandidateProfile, optimized: OptimizedResume) -> bytes:
    ...
    document.core_properties.subject = "Evidence-grounded tailored resume generated by MeritLoom"
...
def improved_resume_pdf(candidate: CandidateProfile, optimized: OptimizedResume) -> bytes:
    ...
    regular_font = "MeritLoomSans"
    bold_font = "MeritLoomSans-Bold"
    ...
    document = SimpleDocTemplate(
        ...
        subject="Evidence-grounded tailored resume generated by MeritLoom",
        allowSplitting=1,
    )
    styles = getSampleStyleSheet()
    styles.add(
...
def meritloom_logo_svg(css_class: str = "hs-logo") -> str:
    return (
        f'<svg class="{css_class}" viewBox="0 0 64 64" role="img" aria-label="MeritLoom logo">'
        ...
        '<path d="M17 45V19L31.5 36L46 19V45" fill="none" stroke="white" stroke-width="5.5" stroke-linecap="round" stroke-linejoin="round"/>'
...
def initialize_state() -> None:
    defaults = {
        ...
        "guide_messages": [
            {
                "role": "assistant",
                "content": "Hi! I’m the MeritLoom Guide. Ask me about analyzing, improving, or downloading your résumé.",
            }
        ],
    }
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="hs-side-brand">
              {meritloom_logo_svg()}
              <div><div class="hs-side-name">MeritLoom</div><div class="hs-side-caption">Smarter, evidence-first applications</div></div>
            </div>
...
def render_tracker() -> None:
    ...
    try:
        ...
        action_cols[1].download_button(
            ...
            file_name="meritloom_application_tracker.csv",
            mime="text/csv",
            width="stretch",
        )
...
def _render_tailor_and_exports(
    ...
    if optimized:
        ...
        st.warning("Review the preview carefully. MeritLoom improves wording but cannot verify facts not stated in your résumé.")
    ...
    export_cols[0].download_button(
        ...
        report_csv(report),
        file_name="meritloom_requirement_evidence.csv",
        mime="text/csv",
        width="stretch",
    )
    try:
        ...
        export_cols[1].download_button(
            ...
            pdf_bytes,
            file_name="meritloom_match_report.pdf",
            mime="application/pdf",
            width="stretch",
        )
...
def render_report(settings: Settings, use_ai: bool) -> None:
    ...
    st.header("MeritLoom Match Report")
    job_line = " · ".join(
        value
        for value in (
            st.session_state.job_title,
...
def render_method() -> None:
    st.markdown('<div class="hs-eyebrow">How MeritLoom works</div>', unsafe_allow_html=True)
    st.header("Transparent matching—not a mystery score")
    st.write(
        "MeritLoom breaks the job description into individual requirements, finds the strongest résumé evidence for "
    ...
    st.warning("MeritLoom is decision support—not an employer ATS score, hiring decision, or legal/immigration opinion.")


def meritloom_guide_response(prompt: str) -> str:
    ...
    if any(term in question for term in ("score", "percentage", "match", "direct", "related", "uncertain", "missing")):
        return "MeritLoom checks every job requirement separately. **Direct** means clearly proven, **Related** means transferable evidence, **Uncertain** needs confirmation, and **Missing** means no specific résumé evidence was found. Required qualifications count more than preferred ones."
    if any(term in question for term in ("tailor", "improve", "rewrite", "resume", "résumé")):
        return "Create a match report first, then open **3 · Tailor & Improve**. MeritLoom improves clarity and supported keyword placement without adding experience, tools, or credentials that are not in your résumé."
    ...
    if any(term in question for term in ("api", "key", "free", "cost", "charge")):
        return "You do **not** need an API key to use MeritLoom. The public app includes deterministic parsing, evidence matching, recommendations, tracking, and document exports."
    if any(term in question for term in ("discover", "linkedin", "indeed", "google", "job search")):
        return "Open **2 · Discover**, enter target titles and locations, and MeritLoom will build direct searches for LinkedIn, Indeed, and Google Jobs."
...
def _append_guide_exchange(prompt: str) -> None:
    ...
    st.session_state.guide_messages.append({"role": "assistant", "content": meritloom_guide_response(cleaned)})
...
def render_guide() -> None:
    ...
    st.header("Ask the MeritLoom Guide")
    st.markdown(
        '<div class="hs-guide-hero"><strong>Instant help—no API key required</strong><span>This guided assistant answers common product and workflow questions using built-in MeritLoom knowledge.</span></div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("guide_question_form", clear_on_submit=True, border=False):
        ...
        submitted = st.form_submit_button("Ask MeritLoom", type="primary", width="stretch")
...
def render_navigation() -> None:
    st.markdown('<div class="hs-nav-label">Explore MeritLoom</div>', unsafe_allow_html=True)
    labels = [("Home", "home"), ("Analyze", "analyze"), ("Discover", "discover"), ("Tracker", "tracker"), ("How it works", "method"), ("Ask MeritLoom", "guide")]
    ...
    st.markdown(
        ...
        <section class="hs-hero">
          ...
          <div class="hs-hero-top">
            <div class="hs-hero-brand">{meritloom_logo_svg("hs-logo hs-logo-hero")}<div><strong>MeritLoom</strong><span>Career Intelligence</span></div></div>
...
def main() -> None:
    ...
    if view == "home":
        ...
        render_guide()
    st.markdown('<div class="hs-footer"><strong>MeritLoom</strong> · Prove your experience. Find your fit.</div>', unsafe_allow_html=True)
